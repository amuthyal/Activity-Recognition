...
index=1; %assignment 2 is just one person
...
disp("Getting dwt of " +users(index)+ " for file " + filename);
...
[eat_coeff,eat_score,eat_latent] = pca(dwt_data);
[non_coeff,non_score,non_latent] = pca(non_dwt_data);
...
fprintf('Using SVM, training 24 %s actions for %s on feature "discrete wavelet" produces\n', utensil,users(index));
...
fprintf('Using DT, training 24 %s actions for %s on feature "discrete wavelet" produces\n', utensil,users(index));
fprintf('\tPrecision %-9.2f\n', confus_precision);
fprintf('\tRecall %-9.2f\n', confus_recall);
fprintf('\tF1-Score %-9.2f\n', confus_f1score);
...
fprintf('Using NN, training 24 %s actions for %s on feature "discrete wavelet" produces\n', utensil,users(index));
fprintf('\tPrecision %-9.2f\n', confus_precision);
fprintf('\tRecall %-9.2f\n', confus_recall);
fprintf('\tF1-Score %-9.2f\n', confus_f1score);
